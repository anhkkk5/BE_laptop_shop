...
export class MockShippingAdapter implements ProviderAdapter {
  ...
  async calculateFee(config: ShippingProviderConfig, input: CalculateShippingFeeDto): Promise<{ fee: number; estimatedDays: number }> {
    const baseFee = 30000;
  ...
  async createOrder(config: ShippingProviderConfig, order: ShippingOrder, address: string): Promise<{ trackingNumber: string; labelUrl: string; estimatedDelivery: Date }> {
    ...
    return { trackingNumber, labelUrl: `https://mock-labels.example.com/${trackingNumber}.pdf`, estimatedDelivery };
  }

  async getTracking(config: ShippingProviderConfig, trackingNumber: string): Promise<{ status: string; history: Array<{ status: string; location: string; description: string; time: Date }>; estimatedDelivery: Date | null }> {
    const now = new Date();
    this.logger.log(`Mock tracking for: ${trackingNumber}`);
    return {
      status: 'in_transit',
      history: [
        { status: 'created', location: 'Kho Hà Nội', description: 'Đơn hàng đã được tạo', time: new Date(now.getTime() - 3600000) },
        { status: 'picked_up', location: 'Kho Hà Nội', description: 'Đã lấy hàng', time: new Date(now.getTime() - 1800000) },
        { status: 'in_transit', location: 'Trung tâm phân loại', description: 'Đang vận chuyển', time: now },
      ],
      estimatedDelivery: new Date(now.getTime() + 2 * 24 * 60 * 60 * 1000),
    };
  }

  async cancelOrder(config: ShippingProviderConfig, trackingNumber: string, reason: string): Promise<void> {

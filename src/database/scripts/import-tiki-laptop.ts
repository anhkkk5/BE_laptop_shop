...
const dataSource = new DataSource({
  ...
  entities: [Brand, Category, Product, ProductImage, User],
});
...
async function runImport() {
  ...
  const [existingBrands, existingCategories, existingProducts] =
    await Promise.all([
      brandRepo.find({ select: ['id', 'name', 'slug'] }),
      categoryRepo.find({ select: ['id', 'name', 'slug', 'parentId'] }),
      productRepo.find({ select: ['id', 'name', 'slug', 'sku'] }),
    ]);

  const usedBrandSlugs = new Set(existingBrands.map((item) => item.slug));
  ...
  const brandNameToId = new Map(
    existingBrands.map((item) => [item.name.trim().toLowerCase(), item.id]),
  );
  const categoryKeyToId = new Map(
    existingCategories.map((item) => [
      `${item.parentId ?? 0}:${item.name.trim().toLowerCase()}`,
      item.id,
    ]),
  );
  const productBySlug = new Map(
    existingProducts.map((item) => [item.slug, item]),
  );
  ...
  for (const item of validItems) {
    ...
    for (const categoryNameRaw of categoryChain) {
      ...
      let categoryId: number | undefined = categoryKeyToId.get(key);
      ...
      if (!categoryId) {
        ...
        const createdCategory: Category = await categoryRepo.save(
          categoryRepo.create({
            name: categoryName,
            slug: categorySlug,
            parentId,
    ...
    const payload: Partial<Product> = {
      ...
      categoryId: parentId,
      brandId,
      status: availability ? ProductStatus.ACTIVE : ProductStatus.OUT_OF_STOCK,
      ...
      specs: item.primary_category_path
    ...
    if (existing) {
      ...
      if (sameSku || sameName) {
        await productRepo.update(existing.id, payload);
        updatedProducts += 1;
      } else {
        const newSlug = buildUniqueSlug(name, 280, usedProductSlugs);
        ...
        productBySlug.set(newSlug, created);
        createdProducts += 1;
      }
      continue;
    }
    ...
    productBySlug.set(slug, created);
    createdProducts += 1;
  }
